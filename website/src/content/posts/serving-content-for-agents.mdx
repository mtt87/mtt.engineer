...
I was inspired by the Cloudflare Blog post ["Markdown for Agents"](https://blog.cloudflare.com/markdown-for-agents/). Their solution requires a Pro, Business or Enterprise plan, so I decided to build it myself.

I built this quickly with Claude Code. Here's the prompt I used in plan mode:
...
If you're behind Cloudflare (as I am, via Cloudflare Tunnel), content negotiation won't work out of the box. Cloudflare caches responses by URL and **ignores the `Vary: Accept` header** — so once the HTML version is cached, all requests get HTML regardless of the `Accept` header.
